...
public class BlogController {
	...
	public ModelAndView details(@PathVariable("id") Integer id, HttpServletRequest request) throws Exception {
		...
		for (int i = 0; i < commentList.size(); i++) {
			System.out.println(commentList.get(i).getUserIp());
		...
		mav.addObject("pageTitle", blog.getTitle() + "博客系统");
	 ...
	 * @throws Exception
	 */
	private String getUpAndDownPageCode(Blog lastBlog, Blog nextBlog, String projectContext) throws Exception {
		StringBuffer pageCode = new StringBuffer();
		if (lastBlog == null || lastBlog.getId() == null) {
			pageCode.append("<p>上一篇：这是第一篇</p>");
		} else {
			pageCode.append("<p>上一篇：<a href=" + projectContext + "/blog/articles/" + lastBlog.getId() + ".html>"
					+ lastBlog.getTitle() + "</a></p>");
		}
		if (nextBlog == null || nextBlog.getId() == null) {
			pageCode.append("<p>下一篇：这是最后一篇</p>");
		} else {
	...
	public ModelAndView search(@RequestParam(value = "q", required = false)String q,@RequestParam(value = "page", required = false)String page, HttpServletRequest request) throws Exception {
		...
		mav.addObject("", "搜索关键字" + q + "结果");
		List<Blog> blogList =  blogIndex.upgradeSearchBlog(q);
//		List<Blog> blogList =  blogIndex.searchBlog(q);
	 ...
	 * @throws Exception
	 */
	private String getUpAndDownPageCode(Integer page, Integer totalNum, String q, Integer pageSize, String projectContext) throws Exception{
		...
		if(totalNum == 0) {
			...
			if(page > 1) {
				pageCode.append("<li class='disabled'><a href='" + projectContext + "/blog/q.html?page="+(page - 1) +"&q ="+q+"'>上一页</a></li>");
			}else {
				pageCode.append("<li class='disabled'><a href='#'>上一页</a></li>");
			}
			if(page < totalPage) {
				pageCode.append("<li class='disabled'><a href='" + projectContext + "/blog/q.html?page="+(page + 1) +"&q ="+q+"'>下一页</a></li>");
			}else {
